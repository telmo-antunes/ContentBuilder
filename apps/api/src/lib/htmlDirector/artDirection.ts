...
THE FORMS, when a brand authors them (the arrangement's own words say which): a ONE-LINER carries a headline and a short tagline and nothing else — never pin it for a slide with a body or rows. An EXHIBIT of figures is for NUMBERS ("40%", "3", "€120") — never for word labels, which break its cells. A NUMBERED poster is for a method or an ordered set. PRODUCT PROOF is for the slide that names a control and carries a screenshot. A CARD holds one claim and its evidence. A STEP CHEVRON is for a short method of two to four steps of a few words. A COMPARE is for a verdict with both do and dont rows, set as two columns. A LEDGER is for three to six numbers with their labels, as a table. A CHECKLIST is for plain items with ticks. Match the form to what the slide SAYS, and prefer that no two consecutive slides share a form.
