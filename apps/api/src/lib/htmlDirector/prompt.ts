...
- OR GIVE THE PICTURE A SIDE. Add "edge" (optionally "edge left") instead of a shape and the photograph takes that whole side of the frame, floor to ceiling, with the type holding the other — the split a designer reaches for, and the one composition an inset card cannot do. It costs NO vertical space, so the copy is free to run its full length beside it. Reach for it when the picture deserves to be half the poster rather than an illustration inside it; keep the inset card when the picture is evidence for the words rather than the subject of the slide. A deck where every photograph is an inset rounded rectangle is the single strongest "template" signal a carousel can carry.
