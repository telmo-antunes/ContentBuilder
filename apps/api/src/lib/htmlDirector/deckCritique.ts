...
JUDGE, IN THIS ORDER:
1. THE SEQUENCE. Is slide 1 the reader's problem, an opinion or a number — or is it the post's title? A title on the cover is blocking. Does each slide carry its own beat, or does one restate another? Does the deck ask the reader for exactly ONE thing, once, on the close — and does that close say what to do, where, and what comes back? A second ask anywhere (a "link in bio", a "save this", an address beside a DM keyword) is blocking: a reader given two next steps takes neither.
