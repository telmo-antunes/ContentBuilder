...
ALIGNMENT IS A DECISION, NOT A DEFAULT. composition.align is read from the brand's evidence, the same way tone is: flush-left suits editorial, utilitarian, engineering-flavoured brands; CENTER is the honest reading of a brand that composes symmetrically — monumental centred type, a centred hero and nav, formal or ceremonial character. The worked examples are flush-left because THOSE brands are, not because flush-left is the house answer — do not inherit it. Whatever you declare, your CSS and fragments must actually build it: a centred system centres its display moments (headline, stat, quote, cta — text-align, auto inline margins on rules and panels, the logo-row placed accordingly) while list rows and any body copy over ~3 lines stay left-aligned, because centred running text is unreadable at the ~3× reduction a phone applies. Declaring one align and authoring the CSS for another is a defect. And the default is not a cage: composition.roles may give individual roles their own alignment — a flush-left brand whose cta centres for the close, a centred brand whose list reads left — exactly as motion.roles gives them their own entrance. The app applies a per-slide alignment layer (data-align on the slide root), so your fragments should not hard-pin text-align on structural elements a role override would need to move.

THE JOB THESE POSTS DO: this brand is a BUSINESS promoting its content and its offer to the audience it serves — a post exists to earn a read, a save, a DM. The strongest posts read like the business THINKS: its own artifacts and instruments (a job sheet, a plan, a verdict, a checklist, a before/after ledger), not abstract editorial about its topic. Author the design system for that register — polished product-grade marketing, unmistakably this brand — and give the composer the component vocabulary it takes:
- a CARD — a framed surface (own ground, border in the brand line colour, the brand radius) that groups a claim's evidence; the single strongest device for making an insight read as the business's own artifact;
- a CHIP / PILL — a small bordered capsule for metadata (a topic, a read time, a label);
- a BADGE — a small FILLED label that sits on a card's corner and names the winner or the point;
- VERDICT ROWS — the app adds class "do" / "dont" to enumeration rows that carry a verdict, draws ✓ against ✕ in the gutter and quiets the losing row (restyle via --cb-do-marker/--cb-do-color/--cb-dont-marker/--cb-dont-color/--cb-dont-opacity); style them so the contrast is legible before the words are;
