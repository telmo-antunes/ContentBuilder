...
- **Seen again:**
  - 2026-09-15 — another deck the same day closed a list slide with "Six things" over **five rows**, and a deck built here announced "Two changes" over a four-row list. Three instances in one day, on two prompt versions, in both directions (fewer items than announced, and more). Confirmed by the owner as open with no check; the direction above is the logged one.
...
- **Direction:** not another lexical check. Two that would bite: (a) let a brief carry NEGATIVE constraints — "the reader's page has already been rebuilt; nothing on it is broken" — threaded into the cta prompt the way `settings.audience` already threads a reader instruction; a constraint written into the brief does reach the copywriter today, so (a) is available now. (b) give the deck critique one copy question for the closing slide: does it assert anything about the reader's own situation that the brief does not state?
- **Correction (2026-09-15, from the owner):** direction (b) as first written assumed the critique reads the deck against the brief. **It does not** — it sees only the contact sheet, and is explicitly told not to touch the meaning of the words. So (b) is not "one more question for a pass that already has the material"; it needs the brief threaded in as a new critique input first. Worth keeping the direction, at its real cost.
- **And (a) has a cost of its own:** the vehicle it recommends — a prohibition written into the brief — is exactly what produced the `you remember` row logged above. The two findings meet here: whoever implements (a) has to separate rules from beats in the same change, or the fix for the invented premise becomes the cause of leaked copy.
...
*Resolved 2026-09-15.* A closing slide shipped with "DM us PAGE" in the
headline, "Send PAGE" on the button and `detailmasters.pro` in the footer: two
different next steps on one slide, and nothing on it saying what the DM was
for or what came back. The owner's rule, verbatim: one carousel, one CTA, and
the CTA must be justified, make sense, and be clear — to whom, for what, how,
when. Checked against the 49 reference carousels in `inspo/`: one closes with
