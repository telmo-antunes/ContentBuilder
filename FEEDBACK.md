...
### A prohibition in the brief came back as a line of slide copy

- **Kind:** Defect
- **Severity:** cost me a fix (it would have shipped as gibberish)
- **First seen:** 2026-09-15 — the-booking-page-before-and-after (IG carousel), slide 6, parse v12
- **What happened:** the idea opened with a hard instruction — *Never write "you remember", never imply the reader noticed a change* — because the whole point of the deck was that the audience has never used the old page. Slide 6 composed as a four-row checklist whose fourth row read, in full: `you remember`. The composer mined the prohibition as material and set the forbidden phrase in the deck, stripped of the "never write" that made it a rule. Every gate passed: it is short, it fits, it is in the brief's own words — `unsourcedWords` in particular *cannot* flag it, since its words come from the brief by construction.
- **Why it matters:** this is not a one-off prompt slip, it is a shape. A brief that says what NOT to say has to contain the forbidden phrasing in order to forbid it, so the more carefully a brief rules something out, the more of that thing sits in the composer's material. Every negative constraint is therefore a small landmine, and the failure is silent — a reader sees a list row, not a leaked instruction.
- **Direction:** separate the two kinds of sentence before the copy is mined. The brief could carry constraints in their own field (`avoid: [...]`) rather than inline prose, so they reach the prompt as rules and never as beats; failing that, a deterministic check — any slide part that appears verbatim inside a sentence beginning "Never", "Do not" or "Don't" in the brief is a copy fault — is cheap and would have caught this exact row.

### `fit: contain` quietly shrinks the evidence when the slot's shape disagrees

- **Kind:** Friction
- **Severity:** cost me a fix (three export-and-look cycles)
- **First seen:** 2026-09-15 — the-booking-page-before-and-after, slides 2-5, all four of them screenshots
- **What happened:** the deck's whole argument is four product screenshots, so they had to be readable at feed size. Attached as `fit: 'contain'` with `shape: 'standard'`, a 1.49:1 screenshot rendered at roughly half the column width, and the layout shrank the box rather than the picture; as `wide` it reached 84%; only after the source PNG was **padded with ground to the slot's own 16:9** did it draw at the full column width. Same asset, same slide, three sizes — decided by a shape attribute whose relationship to the image was never surfaced. The deck critique saw the symptom ("reproduced at about a third of the frame's width") and, correctly, could not name the cause.
- **Why it matters:** every repurpose of a blog post is screenshots in slots — this is the common path, not an edge case. And the failure is the quiet kind: the export looks fine, it is just small, so the deck ships with its evidence illegible unless someone measures a picture against the column.
- **Direction:** when `fit` is `contain`, the slot has no reason to keep a shape the image will not fill — derive the box from the asset's own ratio, or have the attach step answer with the shape nearest the asset and say so. Either removes the guesswork; today the caller has to know that `contain` + a mismatched `shape` means "draw it smaller".
- **Seen again:**
  - 2026-09-15 (same deck, second pass) — and the rule turns out to be sharper than "match the shape". A 1.15:1 montage in a 4:3 box drew at just over half the column; the fix was not padding it to 4:3 but **cropping it to 1.34** — because the box is sized by the height the COPY leaves it, and the width follows from the ratio. So the image wants to be *wider* than the box, not equal to it: at 1.34 in a 1.33 box it filled the column, and cutting two lines of copy widened it again. Nothing in the tool says this, and it is the opposite of what "contain" suggests.
...
- **Severity:** blocked shipping — the deck was rejected on read
- **First seen:** 2026-08-25 — the-aftercare-message (IG carousel), rejected by Telmo: "super generic, doesn't convey the message of the blog post, nor communicates clearly in any way"
- **What happened:** the composed deck said "A real wash date, not vague advice" and "What to avoid, written down" — abstractions ABOUT advice rather than the advice — over two CRM feature slides. The composer was not at fault. The `idea` it received contained a headline, a subhead, a reader line, **two facts, both of them CRM product claims**, and the instruction *"Do not invent CLAIMS of any kind: no facts, no list items the source above does not contain."* None of the post's actual teaching reached it. `buildIdea` assembles a beats block carrying every section heading AND its sentences, but over the 2000-char cap it dropped that block **wholesale** and kept the claims — this post's beats were 1376 chars against a 924-char remainder, so 2335 > 2000 and all six sections went. Given two product claims as its only concrete material and a ban on inventing anything else, writing around the product was the only move available.
- **Why it matters:** the incentive was inverted — the more a post actually taught, the longer its beats, the likelier they were dropped, so **the richest posts produced the most generic decks**. It also explains a whole class of "compose writes vague copy" complaints that were previously read as model weakness. A second bug compounded it: this house style announces a count and enumerates ("It needs four things." + four bullets), and the character clip delivered the promise plus one item — while the same idea tells the composer never to drop a list item, which it cannot honour if the brief itself arrives truncated.
- **Resolved upstream:** 2026-08-28, CRM commit `2d35cd0`. The cap is now met by shortening beats progressively (180→40 chars), then headings alone, each fallback reported; and bulleted bodies are summarised by their items' lead-ins so lists arrive complete. The rebuilt deck carries all three handover problems, all four things the message needs, all four writing rules and all three exclusions.
- **Left for ContentBuilder:** compose has no way to verify that the brief it was handed actually contains substance. `buildIdea` refuses a brief with no beats AND no claims, but a brief with claims and no beats — exactly this case — passes. A compose that receives only product claims as its material is worth a warning on the review page.
...
- **Correction (2026-08-28):** this entry originally read as the explanation for the deck being weak. It was not — the copy failure was upstream (see the brief-substance entry at the top of this file). The slot gap below is real and still open, but it costs manual steps, not quality.
...
- **Seen again:**
  - 2026-09-15 — the-booking-page-before-and-after. The brief could not have been more explicit — *"Slides 2-5 are the evidence, ONE screenshot each … so each of those slides needs a photo slot and short copy"* — and compose authored slots on three of the four. Slide 4, whose headline is *"The row opened. The page moved."*, came back with no `data-cb-slot` at all and `attachPoolPhotos` furnished it from the brand pool: a photograph of a car park ceiling, under a headline about a UI behaviour. So the gap now costs more than manual steps — a slide that asked for evidence got decoration, and nothing in the response distinguished the two. Worth pairing with the `purpose: 'evidence'` idea at the top of this file: a slide whose copy is about the product should not be fillable from the photography pool.
...
- **Seen again:**
  - 2026-08-21 (later) — Telmo calibrated the bar with three hand-designed candidates for the smoke-odour deck's statement slide and picked two: a "job sheet" card (the insight as ticked line items in the product's own vocabulary) and a "split verdict" (masking vs extracting, the winner carrying the accent border and a badge). The common thread: posts by a SaaS promoting a guide should read like the business THINKS, not like editorial about its topic. Shipped in response — not as those two layouts, but as the missing primitives + register: parse rows carry a verdict state ('do'/'dont', v4), the app draws tick/cross and quiets the loser (brand-restylable via --cb-do-*/--cb-dont-* vars), the composer carries state as a row class (v4), and the author prompt (v6) names the job register and demands the vocabulary — card, chip, badge, verdict rows, split-with-winner — with the critique (v5) judging it as point 10.
  - 2026-08-21 — directions (a) and half of (b) implemented. `fragments[role]` accepts variant arrays (validated/carried per variant; the measured gap filler deliberately skips arrays — a variant's identity is what it leaves out); the variant is picked by the same rotation index as the composition pattern, with a per-project seed (`variantSeed`, a stable hash of the project id) so consecutive decks compose different skeletons; the app gained a numbered-rows contract (`numbered` class + `--cb-row-index-*` vars) so recipes can finally express a list beyond the quiet marker panel. Author v5 requires variants on statement/feature/list and the richer list vocabulary; critique v4 judges it. STILL OPEN: the three exemplars show single-string fragments — prose teaches the array shape, but a worked exemplar variant would teach it better (the Halftone lesson, a third time); and detailmasters' stored recipe has one arrangement per role until re-authored, so existing brands see no variety yet.
